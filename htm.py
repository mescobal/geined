...
def botones(url):
    ...
    print(table(tr(td(submit("Aceptar", 'height: 40px; width: 150px'), 
                      align="center") +
     td(button("Cancelar", url , 'height: 40px; width: 150px'), 
        align="center")), '100%', clase='tform'))
...
def encabezado_informe(titulo):
    """Imprime un encabezado, pero haciendo referencia a una hoja de estilo 
    para informes"""
    inicio()
    title(titulo)
...
def form_edicion(texto, accion):
    ...
    print("<div align='center'><form action='" + accion + \
        "' method='post'><table class='tform'><caption>" + \
        texto + "</caption><tr>")
    """ print("<form action='" + accion + "' method='post'><table><caption>" 
    + texto + "</caption><tr>")"""
...
def input_combo(texto, campo, resultado, campos, valor):
    ...
    if valor == '':
        print('<option value="" selected="selected">Sin datos</option>')
...
def seleccionar_ano(defecto=""):
    ...
    for n in range(ano_actual, ano_actual-10, -1):
        if str(n) == seleccionado:
            print("<option selected value='" + str(n) + "'>" + str(n) + "</option>")
        else:
            print("<option value='" + str(n)+ "'>" + str(n) + "</option>")
    print("</select>")
...
def input_input(tipo, nombre, valor=''):
    ...
    print('<input type="'+ tipo + '" name="' + nombre + '" value="'+ valor + \
        '"></input>')
def input_check(texto, variable, valor):
    ...
    print '<TR>'
    print(td(texto))
    print(td("<INPUT TYPE='checkbox' NAME='" + variable + "' VALUE='1' " + \
        adicional + ">"))
    print("</tr>")
def fila_alterna(i):
